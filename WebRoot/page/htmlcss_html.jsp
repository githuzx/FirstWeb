...
for(var i=1;i<=10;i++)
{
	...
	<!-- img标签 -->
	<p>在网页的制作中为使网页绚丽美观,肯定是缺少不了图片,可以使用img标签来插入图片。讲解:src标志图像的位置;alt指定图像的描述性文本,当图像不可见时(下载不成功时),可看到该属性指定的文本;title提供在图像可见时对图像的描述(鼠标滑过图片时显示的文本);图像可以是GIF、PNG、JPEG格式的图像文件。</p>
	<p>
		1922年的春天，一个想要成名名叫尼克•卡拉威（<a href="http://www.m1905.com/mdb/star/3316/"
